...
$sql='select * from item where partsID='.$_POST['item'];
$rst=selectData(DB_NAME,$sql);

$body.='<select id="ponpon" multiple class="form-control form-inline">';
for($i=0;$i<count($rst);$i++){
  $body.='<option value="'.$rst[$i]['id'].'">'.$rst[$i]['itemname'].'</option>';
}
$body.='</select>';
$body.='</div>';
